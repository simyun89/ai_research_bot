...
keyword_prompt = (
    ...
    "이를 바탕으로 **미래 전략 수립에 참고할 수 있는 최근 주요 트렌드나 이슈를 반영한 '한국어 키워드 총 16개'를 순서 상관없이 추천**해주세요.\n"
    "- 필수로 포함하는 키워드는 '자율주행', '모빌리티', '교통정산'입니다 \n   "
    "- 필수 키워드 이외 추천 키워드는 **일반 명사 8개** , 뉴스 검색이 잘 되는 수준의 **적절한 단어 조합 5개** 를 추천해주세요 (예: '모바일결제', '대중교통', '교통카드', '시외버스', '교통 요금' ,'NFC 충전','교통 정기권', '스테이블코인' 등).\n"
