...
keyword_prompt = (
    ...
    "이를 바탕으로 **미래 전략 수립에 참고할 수 있는 최근 주요 트렌드나 이슈를 반영한 '한국어 키워드 8개'를 추천**해주세요.\n"
    "- 키워드는 **검색 가능한 일반 명사 4개** , 뉴스 검색이 잘 되는 수준의 **적절한 단어 조합 4개** 를 추천해주세요 (예: '모바일결제', '대중교통', '교통카드', '시외버스', '교통 요금' ,'NFC 충전','교통 정기권' 등).\n"
    "- 너무 좁거나 합성도가 높은 키워드는 피해주세요.\n"
    "- 키워드는 **쉼표로 구분된 한 줄 리스트**로만 작성해주세요.\n"
    "- 다음 키워드는 반드시 제외해주세요: '빅데이터', '합성데이터', '기후동행', '정산시스템' , '티머니'    \n"
)
        ...
        "messages": [
            {"role": "user",
             "content": f""" '{kw}' 관련 최근 7일 뉴스나 정보를 표 형식으로 정리해줘
                        ...
                        ※ 주의: 최근 7일이내 뉴스 & 링크 이어야하며 원문링크를 반드시 포함하고 실주소가 https:// 로 시작하는 완전한 링크여야 합니다
...
1. **뉴스 요약**
   - **절대로 표 형태로 작성하세요. 우리 회사와 관련 혹은 중요도가 높다고 생각하는 뉴스를 순서대로 나열해주세요 나열 갯수는 최대 15개로 제한합니다.
